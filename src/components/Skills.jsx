...
const levelLabels = ["Basic", "Proficient", "Expert"];

const Skills = ({ isDarkMode }) => {
  // Map original 1–5 levels to 1–3 for Basic, Proficient, Expert
  const mapLevelTo3 = (level) => {
    if (level <= 2) return 1;       // Basic
    if (level === 3) return 2;      // Proficient
    return 3;                        // Expert
  };
          ...
          <div key={index}>
            ...
            >
              {group.items.map((skill, i) => {
                const level = mapLevelTo3(skill.level);
